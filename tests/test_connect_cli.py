...
def test_run_connect_cli_retries_transient_exchange_errors(monkeypatch, tmp_path, capsys):
    private_key = tmp_path / "id_ed25519"
    public_key = tmp_path / "id_ed25519.pub"
    private_key.write_text("PRIVATE", encoding="utf-8")
    public_key.write_text("ssh-ed25519 AAAATESTKEY local", encoding="utf-8")
    ...
    monkeypatch.setattr(core, "_resolve_control_plane_bearer", lambda: "ara_api_key_test")
    monkeypatch.setattr(core, "_ensure_local_ssh_keypair", lambda: (private_key, public_key))
    monkeypatch.setattr(core, "_upsert_ssh_config", lambda alias, block: tmp_path / "ssh_config")
    monkeypatch.setattr(core, "_write_proxy_token_file", lambda token: tmp_path / "proxy_token")
    monkeypatch.setattr(core.time, "sleep", lambda seconds: None)

    attempts = {"count": 0}

    def _fake_exchange(self, *, token: str, public_key: str, key_comment: str):
        _ = (self, token, public_key, key_comment)
        attempts["count"] += 1
        if attempts["count"] == 1:
            raise RuntimeError('POST /session/connect/exchange failed (503): {"detail":"sandbox_unavailable"}')
        return {
            "host_alias": "ara-personal",
            "proxy_token": "proxy_test_1",
            "session_id": "sess-123",
